// Hacıyatmaz Kablo — QNB eSolutions (eFinans) fatura adaptörü.
...
function xmlTagValue(xml, tag) {
  ...
  const match = String(xml || '').match(new RegExp(`<(?:(?:[\\w.-]+):)?${escaped}(?:\\s[^>]*)?>([\\s\\S]*?)<\\/(?:(?:[\\w.-]+):)?${escaped}>`, 'i'));
...
function faturaKalemleri(order) {
  ...
  const kdvOran = 20;                             // Genel oran: elektronik kablo
  ...
  return {
    ...
    ad: 'Hacıyatmaz Kablo Tip C 240W',
  };
}
...
function ublFaturaXml(env, order, faturaNo, uuid) {
  ...
  const aliciKimlik = order.customerIdentityNumber || '11111111111';
  const unvan = env.SATICI_UNVAN || 'Batu Elektronik';
  const vd = env.SATICI_VERGI_DAIRESI || 'Hasanpaşa Vergi Dairesi Müdürlüğü';

  // Bireysel alıcı TCKN bildirmediyse e-Arşiv anonim alıcı değeri kullanılır.
...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2">
  ...
  <cbc:Note>Odeme haciyatmazkablo.com uzerinden iyzico ile alinmistir.</cbc:Note>
  ...
  <cac:AccountingSupplierParty>
    <cac:Party>
      ...
      <cac:PostalAddress>
        <cbc:StreetName>${xmlEsc(env.SATICI_ADRES || 'Bahçelievler Mah. Melikgazi 2. Sk.')}</cbc:StreetName>
        <cbc:BuildingNumber>${xmlEsc(env.SATICI_BINA_NO || '2B')}</cbc:BuildingNumber>
        ...
        <cbc:PostalZone>${xmlEsc(env.SATICI_POSTA_KODU || '19000')}</cbc:PostalZone>
      ...
      <cac:Contact><cbc:Telephone>${xmlEsc(env.SATICI_TELEFON || '05534759032')}</cbc:Telephone><cbc:ElectronicMail>${xmlEsc(env.SATICI_EPOSTA || 'haciyatmazkablo@gmail.com')}</cbc:ElectronicMail></cac:Contact>
    </cac:Party>
  </cac:AccountingSupplierParty>
  <cac:AccountingCustomerParty>
    <cac:Party>
      <cbc:WebsiteURI>https://www.haciyatmazkablo.com</cbc:WebsiteURI>
      <cac:PartyIdentification><cbc:ID schemeID="TCKN">${xmlEsc(aliciKimlik)}</cbc:ID></cac:PartyIdentification>
      <cac:PartyName><cbc:Name>${xmlEsc(order.customerName || '-')}</cbc:Name></cac:PartyName>
      <cac:PostalAddress>
        <cbc:StreetName>${xmlEsc(order.customerAddress || '-')}</cbc:StreetName>
        <cbc:CitySubdivisionName>${xmlEsc(order.customerTown || '-')}</cbc:CitySubdivisionName>
        <cbc:CityName>${xmlEsc(order.customerCity || '-')}</cbc:CityName>
        <cac:Country><cbc:Name>Türkiye</cbc:Name></cac:Country>
      </cac:PostalAddress>
      <cac:Contact><cbc:Telephone>${xmlEsc(order.customerPhone || '')}</cbc:Telephone><cbc:ElectronicMail>${xmlEsc(order.customerEmail || '')}</cbc:ElectronicMail></cac:Contact>
    </cac:Party>
  </cac:AccountingCustomerParty>
  <cac:TaxTotal>
...
async function wsLogin(env) {
  ...
  const fault = xmlTagValue(text, 'faultstring') || xmlTagValue(text, 'Fault');
  if (res.status >= 400 || fault) {
    const f = fault || `HTTP ${res.status}`;
...
async function belgeGonderExt(env, cookie, ublXml, faturaNo) {
  ...
  const belgeOid = xmlTagValue(text, 'belgeOid');
...
export async function qnbIrsaliyeliFaturaKes(env, order) {
  ...
  const faturaNo = `${env.QNB_FATURA_SERI || 'HKT'}${yil}${String(Date.now()).slice(-9)}`;

  const gercek = env.QNB_MOCK !== '1'
    && env.QNB_WS_KULLANICI && env.QNB_WS_SIFRE
    && env.QNB_EFATURA_USER_WS && env.QNB_EFATURA_CONNECTOR_WS
    && env.QNB_GONDERICI_VKN && env.QNB_ERP_KODU;

  if (env.QNB_MOCK === '1') {
    console.log(`[fatura] MOCK irsaliyeli fatura → ${faturaNo} (${env.SATICI_UNVAN || 'Batu Elektronik'} → ${order.customerName}, ${order.amount} ${order.currency})`);
    return {
  ...
  if (!gercek) {
    return { ok: false, manual: true, error: 'QNB e-Arşiv yapılandırması eksik' };
  }
