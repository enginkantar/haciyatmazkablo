...
// Doğrulanmış akış (hydrozidtr.com, 18 Tem canlı):
//   1) POST /shipments → taslak, id döner
//   2) GET  /shipment-price-comparison/{id} → sağlayıcı+fiyat listesi
//   3) POST /confirm-shipping-price → gönderi hazır
//   4) GET  /shipments/{id} → takip kodu (asenkron dolabilir)
// Sağlayıcı: KARGONOMI_SAGLAYICI slug pinliyse o (örn. 'hepsijet'), yoksa en ucuz.
async function kargonomiGonderiOlustur(env, order) {
  ...
  try {
    ...
    // 2) Taslak gönderi — sender_* alanları zorunlu, telefonlar 0'sız 10 hane,
    // sender_tax_number checksum'lı gerçek VKN/TCKN olmalı (canlı 422 testleri).
    ...
    const olustur = await kargonomiCagri(env, 'POST', '/shipments', {
      shipment: {
        ...
        packages: [{
          content: kisalt(`Haciyatmaz Kablo Tip C 240W - ${order.orderNo || order.basketId || order.invoiceId || ''}`, 100),
          desi: 1,
        }],
      },
    });
